...
export default async function BlogPostPage({
  ...
  const content = BLOG_CONTENT[slug] || [
    ...
    'Toz Yapı Teknolojileri olarak 18+ yıllık deneyimimizle her türlü yapı projenizde yanınızdayız.',
  ]
    ...
    <div>
      ...
      <div className="bg-navy-900 py-20 relative overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(ellipse at 40% 50%, #C8A96E, transparent 60%)',
          }}
        />
        <div className="relative max-w-4xl mx-auto px-4">
          ...
          <span className="inline-block text-xs px-2.5 py-1 bg-gold-500 text-navy-900 font-semibold rounded-full mb-4">
            {post.category}
          </span>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="mt-10 pt-8 border-t border-gray-200">
              <div className="flex flex-wrap gap-2 mb-8">
                {post.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 text-sm px-3 py-1 bg-gray-100 border border-gray-200 rounded-full text-gray-600"
                  >
                    <Tag size={12} />
                    {tag}
                  </span>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {related.map((p) => (
                  ...
                  >
                    <span className="text-xs px-2 py-0.5 bg-gold-500/10 text-gold-600 rounded-full font-medium">
                      {p.category}
                    </span>
                    <h4 className="font-semibold text-navy-800 text-sm mt-2 leading-snug group-hover:text-gold-600 transition-colors">
                      {p.title}
                    </h4>
                  </Link>
                ))}
          ...
          <aside className="space-y-6">
            <div className="bg-navy-800 rounded-2xl p-6 text-white sticky top-24">
              ...
              <a
                href={WHATSAPP_URL(`${post.title} hakkında bilgi almak istiyorum.`)}
                target="_blank"
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              <h4 className="font-semibold text-navy-800 mb-4 text-sm">
                Son Yazılar
              </h4>
